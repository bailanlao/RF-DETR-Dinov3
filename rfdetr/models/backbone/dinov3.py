...
from .dinov3_with_windowed_attn import WindowedDinov3WithRegistersConfig, WindowedDinov3WithRegistersBackbone
...
class DinoV3(nn.Module):
    def __init__(self,
        ...
        name = f"facebook/dinov3-with-registers-{size}" if use_registers else f"facebook/dinov3-{size}"
        ...
        if not use_windowed_attn:
            ...
            # print(windowed_dino_config)
            self.encoder = WindowedDinov3WithRegistersBackbone.from_pretrained(
                name,
                config=windowed_dino_config,
            ) if load_dino_weights else WindowedDinov3WithRegistersBackbone(windowed_dino_config)
    ...
    def forward(self, x):
        block_size = self.patch_size * self.num_windows # 14*4=56
