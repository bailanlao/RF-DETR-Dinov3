...
class Model:
    ...
    def train(self, callbacks: DefaultDict[str, List[Callable]], **kwargs):
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            map_regular = test_stats["coco_eval_bbox"][1]
            _isbest = best_map_holder.update(map_regular, epoch, is_ema=False)
            if _isbest:
                best_map_50 = max(best_map_50, map_regular)
                best_map_5095 = max(best_map_5095, test_stats["coco_eval_bbox"][0])
            ...
            if args.use_ema:
                ...
                map_ema = ema_test_stats["coco_eval_bbox"][1]
                best_map_ema_50 = max(best_map_ema_50, map_ema)
                _isbest = best_map_holder.update(map_ema, epoch, is_ema=True)
                if _isbest:
                    best_map_ema_5095 = max(best_map_ema_5095, ema_test_stats["coco_eval_bbox"][0])
