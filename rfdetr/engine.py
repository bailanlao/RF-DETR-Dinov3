...
def coco_extended_metrics(coco_eval):
    ...
    for k, cid in enumerate(cat_ids):
        class_prec = prec[:, k]
        f1_scores = 2 * class_prec * rec_thrs / (class_prec + rec_thrs + 1e-8)
        valid_f1 = f1_scores[~np.isnan(f1_scores)]
        if len(valid_f1) == 0:
            continue
        best_j = np.nanargmax(f1_scores)
        class_precision = float(class_prec[best_j])
        class_recall = float(rec_thrs[best_j])
        class_f1 = float(f1_scores[best_j])
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, args=None):
    ...
    if coco_evaluator is not None:
        ...
        for class_result in results_json["class_map"]:
            if class_result["class"] != "all":
                print(f"Class: {class_result['class']}, mAP@50: {class_result['map@50']:.4f}, Precision: {class_result['precision']:.4f}, Recall: {class_result['recall']:.4f}, f1: {class_result['f1']:.4f}")
        all_result = [r for r in results_json["class_map"] if r["class"] == "all"][0]
        print(f"All classes - mAP@50: {all_result['map@50']:.4f}, Precision: {all_result['precision']:.4f}, Recall: {all_result['recall']:.4f}, f1: {all_result['f1']:.4f}")
